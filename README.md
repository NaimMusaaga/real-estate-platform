...
- **A schema that only worked on the old MariaDB.** The first production deploy failed at migration time: `listing_photos.cover_slot` (a generated column enforcing "one cover photo per listing") was built from `listing_id`, which also sits in an `ON DELETE CASCADE` foreign key. MariaDB 10.4 (local XAMPP) tolerated that; the 10.6 that Ubuntu ships rejects it, even as `VIRTUAL`. Fixed by deriving the column from `is_cover` alone and moving the uniqueness to a composite `(listing_id, cover_slot)` key — verified on both versions that it allows many regular photos, one cover per listing, and rejects a second cover.
